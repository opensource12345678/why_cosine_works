...
def parse_argument(sys_argv):
    ...
    parser.add_argument(
        '--checkpoint_name', default='checkpoint', type=str,
        help='The checkpoint basename')
    parser.add_argument(
...
def load_data(dataset_name, batch_size):
    ...
    elif dataset_name == 'imagenet':        # ImageNet: ILSVRC2012
    ...
    stat_dict['num_sample'] = len(training_set)
...
def optimize_in_pytorch(train_loader, test_loader, num_class,
    ...
    elif model_type in ['resnet18', 'resnet34', 'resnet50', 'resnet152']:
        model = models.__dict__[model_type](num_classes=num_class)
    else:
    ...
    def test_on(data_loader):
        ...
        for x_batch, y_batch in data_loader:
            ...
            # Top-5 accuracy
            _, pred = predicted_logits.topk(
                k=5, dim=1, largest=True, sorted=True)
            pred = pred.t()             # [Batch, Topk] -> [Topk, Batch]
            top5_correct = pred.eq(y_batch.view(1, -1).expand_as(pred))
            num_top5_correct += top5_correct.reshape(-1).sum()

        num_sample = len(data_loader.dataset)
        test_loss = sum_test_loss / float(num_sample)
        test_acc = num_correct.float() / float(num_sample)
        test_top5_acc = num_top5_correct.float() / float(num_sample)
    ...
    for epoch_id in range(args.num_epoch):
        ...
        for x_batch, y_batch in train_loader:
            ...
            if True:
                logging.info('t = %d, learning rate: %.10lf', t, learning_rate)
                logging.info('t = %d, batch loss: %.10lf', t, batch_loss)
                logging.info('t = %d, batch loss with l2: %.10lf',
                             t, batch_loss_with_l2)
        ...
        test_loss, test_acc, test_top5_acc = test_on(test_loader)
        ...
        if args.checkpoint_dir:
            ...
            checkpoint_state = {
                ...
                'train_loss_with_l2': train_loss_with_l2,
                'test_loss': test_loss,
                'test_acc': test_acc,
    ...
    train_loss, train_acc, train_top5_acc = test_on(train_loader)

    logging.info('Summary: train loss: %.10lf, train accuracy: %.6lf'
                 ', train top-5 accuracy: %.6lf',
