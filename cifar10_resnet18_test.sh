...
function run_hyperparam_search() {
  ...
  local log_dir="log/cifar10/${model_type}_test"
  local conf_dir="conf/cifar10/${model_type}_test"
  ...
  for init_lr in 0.1; do
    ...
    for min_lr in 0; do
      ...
      python3 python/scripts/preprocess_eigenval.py \
        --input_file tmp/sorted_eigenvalue_iter-5000.txt \
        --abs \
        --output_file tmp/sorted_eigenvalue_iter-5000.abs.scale.txt \
        > /dev/null

      python3 python/scripts/gen_eigen_dependent_schedule_conf.py \
        --input_eigenval_file tmp/sorted_eigenvalue_iter-5000.abs.scale.txt \
...
EOF
  ...
  for init_lr in 0.1; do
    # Skips if have experiment records before
    local prefix="step-decay_resnet_piecewise_epoch-${num_epoch}_0.33-epoch-0.1_0.66-epoch-0.01_run-${run}"
    if [ -f ${log_dir}/${prefix}.log ]; then
      echo "$(date):          have records before, skipped"
      continue
    fi
